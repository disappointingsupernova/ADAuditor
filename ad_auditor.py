    ...
    for group in conn.entries:
        ...
        for member_dn in members:
            ...
            if username:
                ...
                if row:
                    if dry_run:
                        log(f"    [DRY-RUN] Would update email/manager for {username}")
                    else:
                ...
                if dry_run:
                    log(f"    [DRY-RUN] Would insert group: {username} -> {group_name}")
                else:
...
This is an automated message generated by the TechOps Team."""
            ...
            if dry_run:
                log(f"[DRY-RUN] Would send email to {manager_email} for user {username}")
                log(f"[DRY-RUN] Would insert audit log for {username} with secret {secret}")
            else:
